...
return (
    <div className="db-container no-horizontal-scroll">
      <h2 className="db-title">System Audit Trail</h2>
        ...
        <div className="responsive-table-wrapper">
          <table className="mgmt-table">
            ...
            <tbody>
              {logs.length > 0 ? logs.map(log => (
                <tr key={log.id}>
                  <td className="mgmt-td" style={{ fontSize: '11px', whiteSpace: 'nowrap' }}>
                    {new Date(log.created_at).toLocaleString()}
                  </td>
                  <td className="mgmt-td"><strong>{log.admin_name}</strong></td>
                  <td className="mgmt-td">
                    <span className="type-subtext">{log.admin_role?.replace('_', ' ')}</span>
                  </td>
                  <td className="mgmt-td">
                    <span className={`status-badge ${getBadgeClass(log.action_type)}`} style={{ fontSize: '10px' }}>
                      {log.action_type.replace(/_/g, ' ')}
                  ...
                  <td className="mgmt-td" style={{ color: '#64748b', fontSize: '13px', minWidth: '250px' }}>
                    {log.details.replace(/ID:?\s?\d+/, log.target_name || 'System')}
                  </td>
                </tr>
              )) : (
